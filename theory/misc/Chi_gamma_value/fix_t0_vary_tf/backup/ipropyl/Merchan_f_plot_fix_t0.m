...
%% total top 1-1000 path
top_1_to_1000_idx = linspace(1, 1000, 1000);
for idx=1:length(top_1_to_1000_idx)
    true_idx = top_1_to_1000_idx(idx);
    if idx == 1
        top_1_to_1000_vec = pp_mat(true_idx, :);
    else
        top_1_to_1000_vec = (top_1_to_1000_vec + pp_mat(true_idx, :));
    end
end
...
% p4 = plot(time_vec * tau, top_3_vec, 'LineWidth', 2, 'marker', '<'); hold on;
% 
% p5 = plot(time_vec * tau, top_4_to_1000_vec, 'LineWidth', 2, 'marker', '>'); hold on;

p6 = plot(time_vec * tau, top_1_to_1000_vec, 'LineWidth', 2, 'marker', 'o'); hold on;
...
% leg_h = legend([p4; p5; p6], 'SUM(p1,p2,p3)', 'SUM(p4,...,p1000)', 'SUM(p1,...,p1000)');
leg_h = legend(p6,'SUM(p1,...,p1000)');
...
text(t_x, t_y, 'ipropyl, $t_0$=0', 'Interpreter','latex', 'FontSize', 20);

%% save to file
figname = strcat('Merchant_f_fix_t0_vary_tf_', end_t, '_S', spe_idx, '_v1.png');
