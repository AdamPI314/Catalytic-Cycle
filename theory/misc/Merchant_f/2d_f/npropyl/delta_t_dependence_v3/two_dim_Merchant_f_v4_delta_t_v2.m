...
elseif length(path_idx) == 3
    str_name{N+1, 1} = 'Merchant 3$\times \beta$';
end
...
elseif length(path_idx) == 3
    h_beta = plot(time_vec(start_idx:end), 3*beta(start_idx:end), 'LineWidth', 2, 'color', colors(idx, :));
end
hold on;
...
%% save to file
figname = strcat('2d_Merchant_f_', end_t, '_S', spe_idx, '_', cycle, '_v4_delta_t_v2.png');
