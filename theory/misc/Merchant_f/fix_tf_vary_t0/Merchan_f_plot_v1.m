...
%% total top 1000 path
top_1000_idx = linspace(1, 1000, 1000);
for idx=1:length(top_1000_idx)
    if idx == 1
        top_1000_vec = pp_mat(idx, :);
    else
        top_1000_vec = (top_1000_vec + pp_mat(idx, :));
    end
end

%% top3_over_top1000
top3_over_top1000_vec = top_3_vec ./ top_1000_vec;
...
%% plot
p1 = plot(time_vec * tau, top_3_vec, 'LineWidth', 2, 'marker', '+'); hold on;
p2 = plot(time_vec * tau, top_1000_vec, 'LineWidth', 2, 'marker', 'o'); hold on;
p3 = plot(nan, nan, 'LineWidth', 2, 'marker', 'd');
...
yyaxis right
delta_n = 1000;
plot(time_vec * tau, 100*top3_over_top1000_vec, 'LineWidth', 2, 'color', 'r', 'marker', 'd'); hold on;
...
%% save to file
figname = strcat('Merchant_f_fix_tf_vary_t0_', end_t, '_v1.png');
