...
for idx=1:3
    r_idx = I(idx);
    if idx == 1
        R_main_cycle = R_mat(:, r_idx);
    else
        R_main_cycle = (R_main_cycle + R_mat(:, r_idx));
    end
end
...
for idx=4:length(I)
    r_idx = I(idx);
    if idx == 4
        R_other_reactions = R_mat(:, r_idx);
    else
        R_other_reactions = (R_other_reactions + R_mat(:, r_idx));
    end
end
...
%% save to file
figname = strcat(spe_name, '_source_reaction_ratio_sum', '.png');
