...
%     spe_idx = spe_idx_vec(I(idx));
    ...
    scatter(time_vec(1:delta_n:end), 1./drc_mat(1:delta_n:end, spe_idx),...
        'LineWidth', 1, 'MarkerEdgeColor', colors(idx, :), 'marker', markers{1, mod(idx-1, length(markers))+ 1}, ...
        'HandleVisibility','off');
    hold on;
    plot(nan, nan, 'LineWidth', 1, 'color', colors(idx, :), 'marker', markers{1, mod(idx-1, length(markers))+ 1});
    hold on;
end
...
%% save to file
figname = strcat(fig_prefix, '_selected_species_v1', '.png');
