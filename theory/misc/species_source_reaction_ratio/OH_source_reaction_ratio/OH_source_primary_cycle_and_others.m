...
for idx=1:length(R_idx)
    r_idx = R_idx(idx);
    if idx == 1
        R_total = reaction_R_mat(:, r_idx);
    else
        R_total = (R_total + reaction_R_mat(:, r_idx));
    end
end

idx_main_cycle = [1162, 1214, 452];
idx_main_cycle = idx_main_cycle + 1;
for idx=1:3
    r_idx = idx_main_cycle(idx);
    if idx == 1
        R_main_cycle = reaction_R_mat(:, r_idx);
    else
        R_main_cycle = (R_main_cycle + reaction_R_mat(:, r_idx));
    end
end

R_other_reactions = R_total - R_main_cycle;
...
%% save to file
figname = strcat(spe_name, '_source_reaction_ratio_primary_cycle_and_others', '.png');
